...
func TestRunCollect_InsertsEventsFromEnabledCollectors(t *testing.T) {
	...
	results, err := pipeline.RunCollect(cfg, s, registry, nil, credentials.Set{}, credentials.Set{}, nil)
...
func TestRunCollect_ReportsUnregisteredCollectorAsNegativeOne(t *testing.T) {
	...
	results, err := pipeline.RunCollect(cfg, s, map[string]func() pipeline.Collector{}, nil, credentials.Set{}, credentials.Set{}, nil)
...
func TestRunCollect_ExcludedTicketKeysPersistedWithoutMutatingTicketKeys(t *testing.T) {
	...
	_, err = pipeline.RunCollect(cfg, s, registry, compiled, credentials.Set{}, credentials.Set{}, nil)
	require.NoError(t, err)
...
func TestRunCollect_NoConfiguredPatternsNeverAddsExclusionArtifact(t *testing.T) {
	...
	_, err := pipeline.RunCollect(cfg, s, registry, nil, credentials.Set{}, credentials.Set{}, nil)
	require.NoError(t, err)
...
func TestRunCollect_JiraCollectorDedupesOnSecondPass(t *testing.T) {
	...
	first, err := pipeline.RunCollect(cfg, s, reg, nil, creds, credentials.Set{}, nil)
	require.NoError(t, err)
	assert.Equal(t, 1, first["jira"], "the status change must be inserted on the first pass")

	second, err := pipeline.RunCollect(cfg, s, reg, nil, creds, credentials.Set{}, nil)
...
func TestRunCollect_JiraEnabledButUnregisteredReportsSentinel(t *testing.T) {
	...
	results, err := pipeline.RunCollect(cfg, s, map[string]func() pipeline.Collector{}, nil, credentials.NewSet(nil), credentials.Set{}, nil)
